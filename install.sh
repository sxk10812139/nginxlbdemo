...
docker run -tid -p 8004:80 -v=$(pwd)/lb.conf:/etc/nginx/nginx.conf -v=$(pwd)/upstream.conf:/etc/nginx/upstream.conf --net mynet --name lb nginx 
docker run -tid -p 8005:80 -v=$(pwd)/flb.conf:/etc/nginx/nginx.conf --net mynet --name flb nginx&&docker ps -a 
...
docker exec s1 /bin/bash -c "apt-get update&&apt-get install -y tcpdump net-tools"
